    ...
    def download_and_apply_update(self, download_url, progress_callback=None):
        ...
        try:
            ...
            with open(update_script, 'w') as f:
                ...
                # Clean up old PyInstaller temp folders that might conflict
                f.write(f'echo Cleaning up old temp files... >> "{update_log}"\n')
                f.write(f'for /d %%i in ("%TEMP%\\_MEI*") do rd /s /q "%%i" 2>nul\n')
                f.write(f'timeout /t 2 /nobreak >nul\n')
                f.write(f'\n')
